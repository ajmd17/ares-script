...
Token Lexer::NextToken()
{
    ...
    if (ch == '\'' || ch == '"') {
        ...
        ReadChar();
        return { Token_open_brace, "{", loc };
    } else if (ch == '}') {
        ReadChar();
        return { Token_close_brace, "}", loc };
    } else if (ch == '[') {
        ReadChar();
        return { Token_open_bracket, "[", loc };
    } else if (ch == ']') {
        ReadChar();
        return { Token_close_bracket, "]", loc };
    } else if (ch == '(') {
        ReadChar();
        return { Token_open_parenthesis, "(", loc };
    } else if (ch == ')') {
        ReadChar();
        return { Token_close_parenthesis, ")", loc };
    } else if (ch == ';') {
        ReadChar();
        return { Token_semicolon, ";", loc };
    } else if (ch == ':') {
        ReadChar();
        return { Token_colon, ":", loc };
    } else if (ch == ',') {
        ReadChar();
        return { Token_comma, ",", loc };
    } else {
        ErrorMsg(Msg_unexpected_token, loc, ReadChar());
        return {};
    }
}
...
Token Lexer::ReadHexNumberLiteral()
{
    ...
    return { Token_integer, ss.str(), loc };
}
...
Token Lexer::ReadStringLiteral()
{
    ...
    if (PeekChar(1) == delimiter && PeekChar(2) == delimiter) {
        ...
        if (ReadChar() != delimiter) {
            ErrorMsg(Msg_unterminated_string_literal, loc);
        }

        return { Token_string, str, loc };
    }
}
...
Token Lexer::ReadIdentifier()
{
    ...
    if (kw == Keyword::Keyword_invalid) {
        return { Token_identifier, str, loc };
    } else {
        return { Token_keyword, str, loc };
    }
}
...
Token Lexer::ReadOperator()
{
    ...
    if (next_three == "...") {
        ReadChar();
        ReadChar();
        return { Token_ellipsis, next_three, loc };
    }

    if (next_two == "->") {
        ReadChar();
        return { Token_right_arrow, next_two, loc };
    } else if (next_two == "<-") {
        ReadChar();
        return { Token_left_arrow, next_two, loc };
    }

    if (IsOperator(next_two)) {
        ReadChar();
        return { Token_operator, next_two, loc };
    } else if (next_two == "/*") {
        ReadChar();
        ReadBlockComment();
        return { Token_unknown, next_two, loc };
    } else if (next_two == "//") {
        ReadChar();
        ReadLineComment();
        return { Token_unknown, next_two, loc };
    ...
    switch (ch) {
    case '.':
        return { Token_period, str, loc };
    default:
        return { Token_operator, str, loc };
    }
}
...
void Lexer::ReadBlockComment()
{
    ...
    ErrorMsg(Msg_unexpected_eof, state.location);
}
...
char Lexer::ParseEscape()
{
    ...
    case '\\':
        return '\\';
    default:
        ErrorMsg(Msg_unrecognized_escape_sequence, loc, ch);
        return '\0';
    }
}
