...
std::unique_ptr<AstModule> Parser::Parse()
{
    ...
    std::string module_name = "__Module";
...
std::unique_ptr<AstNode> Parser::ParseImport()
{
    ...
    if ((local_path + value) == filepath) {
        ErrorMsg(Msg_import_current_file, tok->location);
        return nullptr;
    }
...
std::unique_ptr<AstNode> Parser::ParseStatement()
{
    if (MatchRead(Token_semicolon)) {
        return std::unique_ptr<AstStatement>(new AstStatement(Location(), main_module));
    }
    ...
    if (Match(Token_keyword)) {
        ...
        if (before &&
            ...
            SourceLocation location(before->location.line,
                before->location.column + before->value.length(), before->location.file);
            WarningMsg(Msg_expected_semicolon, location);
        }
...
std::unique_ptr<AstNode> Parser::ParseVariableDeclaration()
{
    ...
    if ((!Peek()) || (Peek()->type != Token_identifier)) {
        ...
        return std::unique_ptr<AstVariableDeclaration>(new AstVariableDeclaration(tok->location, main_module,
...
std::unique_ptr<AstNode> Parser::ParseAlias()
{
    ...
    if ((!Peek()) || (Peek()->type != Token_identifier)) {
        ...
        if (MatchRead(Token_operator, BinaryOp_ToString(BinOp_assign)) || MatchRead(Token_colon)) {
            ...
            if (Match(Token_identifier) && (alias_to = ParseIdentifier())) {
                return std::unique_ptr<AstAlias>(new AstAlias(tok->location, main_module, identifier, std::move(alias_to)));
            } else {
                ErrorMsg(Msg_alias_must_be_identifier, tok->location, identifier);
            }
        } else {
            ErrorMsg(Msg_alias_missing_assignment, tok->location, identifier);
        }
    }
    return nullptr;
}
...
std::unique_ptr<AstNode> Parser::ParseUsing()
{
    ...
    if (MatchRead(Token_keyword, Keyword_ToString(Keyword_module))) {
        ...
        for (int i = 0; Peek(i) && Peek(i)->type == Token_identifier;) {
            identifier = Read()->value; ++i;
            if (Peek(i) && MatchRead(Token_period)) {
                ++i;
            }
        }
        ...
        for (int i = 0; Peek(i) && Peek(i)->type == Token_identifier;) {
            ...
            if (Peek(i) && Peek(i)->type == Token_period) {
                ++i;
            }
        }
        return std::unique_ptr<AstAlias>(new AstAlias(tok->location, main_module, identifier, std::move(ParseIdentifier())));
    } else {
        ErrorMsg(Msg_expected_identifier, Location());
    }
    return nullptr;
...
std::unique_ptr<AstNode> Parser::ParseUnaryOp()
{
    ...
    return std::unique_ptr<AstUnaryOp>(new AstUnaryOp(tok->location, main_module,
        std::move(value), op));
}
...
std::unique_ptr<AstNode> Parser::ParseClass()
{
    ...
    return std::unique_ptr<AstClass>(new AstClass(tok->location, main_module,
...
std::unique_ptr<AstNode> Parser::ParseNew(const std::string &identifier)
{
    ...
    std::unique_ptr<AstNode> class_type;
    if (Match(Token_identifier)) {
        class_type = std::move(ParseIdentifier());
    } else {
        ErrorMsg(Msg_unexpected_token, Location(), Read()->value);
    }

    return std::unique_ptr<AstNew>(new AstNew(tok->location, main_module,
        identifier, std::move(class_type)));
}
...
std::unique_ptr<AstNode> Parser::ParseTerm()
{
    ...
    if (Match(Token_open_parenthesis)) {
        ...
        term = std::move(ParseIntegerLiteral());
    } else if (Match(Token_float)) {
        term = std::move(ParseFloatLiteral());
    } else if (Match(Token_identifier)) {
        term = std::move(ParseIdentifier());
    } else if (Match(Token_string)) {
        term = std::move(ParseStringLiteral());
