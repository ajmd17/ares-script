...
private:
    ...
    {
        state.errors.push_back(BuildMessage(type, Level_fatal, loc, args...));
    }

    template <typename ... Args>
    void WarningMsg(ErrorType type, SourceLocation loc, Args && ... args)
    {
        state.errors.push_back(BuildMessage(type, Level_warning, loc, args...));
    }

    template <typename ... Args>
    void InfoMsg(ErrorType type, SourceLocation loc, Args && ... args)
    {
        state.errors.push_back(BuildMessage(type, Level_info, loc, args...));
    }
};
} // namespace avm
