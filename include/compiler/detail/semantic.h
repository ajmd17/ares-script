...
private:
    ...
    {
        state_ptr->errors.push_back(BuildMessage(type, Level_fatal, loc, args...));
    }

    template <typename ... Args>
    void WarningMessage(ErrorType type, SourceLocation loc, Args && ... args)
    {
        state_ptr->errors.push_back(BuildMessage(type, Level_warning, loc, args...));
    }

    template <typename ... Args>
    void InfoMsg(ErrorType type, SourceLocation loc, Args && ... args)
    {
        state_ptr->errors.push_back(BuildMessage(type, Level_info, loc, args...));
    }
};
} // namespace avm
